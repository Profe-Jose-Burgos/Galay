...
class Chat(object):
    def __init__(self, pairs=(), reflections=None, call=_function_call,
                 api=None, normalizer=None, default_template=None, language="es", local_path=None):
        """
        Initialize the chatbot.  Pairs is a list of patterns and responses.  Each
        pattern is a regular expression matching the user's statement or question,
        e.g. r'I like (.*)'.  For each such pattern a list of possible responses
        is given, e.g. ['Why do you like %1', 'Did you ever dislike %1'].  Material
        which is matched by parenth# Printing the string "es" to the console.
        esized sections of the patterns (e.g. .*) is mapped to
...
def demo(first_question=None, language="es", gui=None, **kwargs):
    if not first_question:
        first_question = FIRST_QUESTIONS.get(language, FIRST_QUESTIONS["es"])
    terminate = TERMINATES.get(language, TERMINATES["es"])
    Chat(language=language, **kwargs).converse(first_question, terminate=terminate, gui=gui)#que es esto?   #chat = Chat(language=language, **kwargs)
