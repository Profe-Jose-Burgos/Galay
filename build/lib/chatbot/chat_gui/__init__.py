...
class ChatGUI:
    ...
    def __init__(self, callback, first_message="Bienvenido al ChatBot", terminate="quit"):#esta es la clase que se encarga de la interfaz grafica
        """
        ...
        self.bot_label = Label(self.root, image=self.bot_image)
    ...
    def close_handler(self): #esta funcion es la que se encarga de cerrar la ventana
        """
    ...
    def show_bubble(self, message="", bot=True): #esta funcion es la que se encarga de mostrar los mensajes en la ventana
        if self.last_bubble:
        ...
        #scroll automatico
        self.canvas.yview_moveto(100)

    def add_icon(self, widget, bot=True):   #Esta funcion agrega el icono de la burbuja de dialogo
        #colocar nombre en la burbuja
    ...
    def draw_triangle(self, widget, bot=True): #Esta funcion dibuja el triangulo de la burbuja de dialogo
        """
        Draw the triangles in the bubble widget.
        """
        #colocar titulo en la burbuja
        self.canvas.create_text(50, 440, text="Asistente virtual", font=("Helvetica", 12), anchor='nw')
        x1, y1, x2, y2 = self.canvas.bbox(widget)
        if bot:
            return x1, y2 - 10, x1 - 10, y2, x1, y2
        return 700, y2 - 10, 700, y2, 710, y2

    def add_user_message(self, message):#esta funcion agrega el mensaje del usuario
        """
    ...
    def add_bot_message(self, message):#esta funcion agrega el mensaje del bot
        """
    ...
    def process_message(self, message):#esta funcion procesa el mensaje del usuario
        """
        Call the bot handler and add the result to bot_message
        """
        bot_message = self.callback(message)
        while not self.thread_event.is_set():
            sleep(0.1) #espera 0.1 segundos para que el usuario pueda ver el mensaje del bot
        self.add_bot_message(bot_message) #agrega el mensaje del bot

    def user_input_handler(self, event): #esta funcion procesa el mensaje del usuario
        """
    ...
    def user_input_box_handler(self, event): #esta funcion procesa el mensaje del usuario
        """
